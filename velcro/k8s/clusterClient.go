...
// (For example, the client config is almost useless without also having
// some structs recalling what API endpoints and versions we've discovered.)
type Client struct {
	cc        *rest.Config
	resources map[string]meta_v1.APIResource
}
...
func LoadClientConfig(filepath string) (cli *Client, err error) {
	...
	cli.resources, err = saneDiscovery(cli.cc)
	if err != nil {
		return
	}
...
// Protorequest returns a 'dynamic.ResourceInterface', which is a partially-baked
// REST request builder which you can then call 'Watch', 'List', etc upon.
//
// n.b. the package you almost certainly want to import for actual structs is
//    "k8s.io/apimachinery/pkg/apis/meta/v1/unstructured"
//
// Cast responses to `*unstructured.Unstructured`.
// Guard this secret well.  Kubernetes wants not to admit to it.
func (cli *Client) Protorequest(kind string, namespace string) dynamic.ResourceInterface {
	// Pick out the relevant APIResource info.
	resourceDesc, ok := cli.resources[kind]
	if !ok {
		panic(fmt.Errorf("no resource called %q available on this server", kind))
	}

	// Massage the REST config first.
	//
	// I'd do a DeepCopy on this so we don't mutate the object we started from, but
